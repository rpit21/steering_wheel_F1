...
void HAL_SPI_Init(void) {
    ...
    IP_PCC->PCCn[LPSPI_PCC_INDEX] |= PCC_PCCn_PCS(3); /* FIRCK DIV2 = 48MHZ */
    IP_PCC->PCCn[LPSPI_PCC_INDEX] |= PCC_PCCn_CGC_MASK;
    ...
    //reset
    //IP_LPSPI0->CR |= LPSPI_CR_RST_MASK;
    //IP_LPSPI0->CR &= ~LPSPI_CR_RST_MASK;

    //IP_LPSPI0 -> CR = 0;
    //while(IP_LPSPI0->CR & LPSPI_CR_MEN_MASK); // Sicurezza

    IP_LPSPI0->CFGR1 = LPSPI_CFGR1_MASTER_MASK;// |LPSPI_CFGR1_NOSTALL_MASK;

    /* Baud 3 MHz */
    IP_LPSPI0->CCR = LPSPI_CCR_SCKDIV(15) | LPSPI_CCR_DBT(2) | LPSPI_CCR_SCKPCS(2) | LPSPI_CCR_PCSSCK(2);

    IP_LPSPI0->TCR =
        LPSPI_TCR_PRESCALE(0) |   // divide by 1
        LPSPI_TCR_FRAMESZ(7)  |   // 8 bits
        LPSPI_TCR_PCS(0)      |   // PCS0 (aunque CS sea GPIO)
        LPSPI_TCR_CPOL(0)     |   // SPI mode 0
        LPSPI_TCR_CPHA(0)     |
        LPSPI_TCR_CONT(0)     |   // continuous
        LPSPI_TCR_LSBF(0);


    /* 4. ENABLE (CRUCIALE: MEN=1 AND RST=1) */
    /* Nel tuo codice mancava LPSPI_CR_RST_MASK qui sotto! */
    IP_LPSPI0->CR = LPSPI_CR_MEN_MASK| LPSPI_CR_DBGEN_MASK;
